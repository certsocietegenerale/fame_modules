...
# Dirty temporary config
# CONFIGURE OR IMPORT THESE VARIABLES
auth_token = "" # cuckoo api token
cuckoo_ip = ""
cuckoo_port = ""
...
class Cuckoo(ProcessingModule):
    ...
    def submit_file(self, filepath, options):
        REST_URL = "http://{}:{}/tasks/create/file".format(cuckoo_ip, cuckoo_port)
        ...
        self.task_id = r.json()["task_id"]

    def submit_url(self, target_url, options):
        REST_URL = "http://{}:{}/tasks/create/url".format(cuckoo_ip, cuckoo_port)
        HEADERS = { "Authorization": "Bearer {}".format(auth_token) }
        data = {"url": target_url}
